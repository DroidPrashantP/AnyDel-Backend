...
class CompletedDeliveryFragment : Fragment() {
    ...
    companion object {
        fun newInstance() = CompletedDeliveryFragment()
    }
    ...
    @BindView(R.id.rv_deliveryCompletedScreen)
    ...
    override fun onCreateView(
        ...
        val fragmentView = inflater.inflate(R.layout.fragment_delivery_completed, container, false)
    ...
    private fun updateUI(packageData: MutableList<PackageInfo>) {
        if (packageData.isListNotEmpty()) {
            ...
            for (packageInfo in packageData) {
                if (AnyDelConstant.DELIVERY_COMPLETED.equals(packageInfo.packStatus, true)) {
        ...
        if (CommonUtility.isNetworkAvailable(mContext)) {
            ...
            mDeliveryCompletedViewModel?.getDeliveryCompletedList(
                PackageListDetailPayload(
                    CommonUtility.getUserData()?.mobileNo.toString(), AnyDelConstant.DELIVERY_COMPLETED
                )
            )
        } else {
    ...
    @OnClick(R.id.btn_deliveryCompletedScreen_home)
    fun clickOnHomeBtn() {
